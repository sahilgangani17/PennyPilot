...
class ExpenseTxnPage extends StatefulWidget {
  const ExpenseTxnPage({super.key});

  @override
  _ExpenseTxnPageState createState() => _ExpenseTxnPageState();
}

class _ExpenseTxnPageState extends State<ExpenseTxnPage> {
  ...
  // Fetching the category-wise expenses from the database
  Future<Map<String, double>> fetchData() async {
    final dbService = DatabaseTxn.instance;
    var categoryData = await dbService.getCategoryWiseExpenses(getCurrentUserEmail()!);
    return categoryData;
  }
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return Column(
            children: [
              ...
              // Display the list of transactions (expenses)
              DisplayTxns(displayTxnType: TxnStates.expensesTxns), // Show the transaction list for expenses
            ],
          );
        }
      },
...
List<PieChartSectionData> createSections(Map<String, double> data, List<Color> colors) {
  ...
  return data.entries.map((entry) {
    return PieChartSectionData(
      value: entry.value,
      // Remove text inside the section
      title: '', // Empty title, no text inside the pie chart section
